...
export default function Reviews() {
    ...
    if (!user || !auth || !reviews) {
        return (
            <>
                <Header />
                <div className="min-h-screen bg-gray-900 text-white flex justify-center items-center">
                    <p className="text-purple-300">Cargando reseñas...</p>
                </div>
            </>
        );
    }
                                    ...
                                    {reviews.data.map((review) => (
                                        <div key={review.id} className="bg-gray-800 p-4 rounded-lg border border-purple-600">
                                            <div className="flex items-start space-x-4">
                                                <Link
                                                    href={review.game?.igdb_id ? route("game.detail", review.game.igdb_id) : "#"}
                                                ...
                                                <div className="flex-1">
                                                    <h3 className="text-xl font-semibold text-neon-green">
                                                        {review.game?.title || "Juego no disponible"}
                                                    </h3>
                                                    ...
                                                    {auth.user && auth.user.id === review.user.id && editingReviewId !== review.id && (
